...
export function RightPanel({ overlay, onUpdate, onRemove }: Props) {
  const definition = overlay ? getOverlayDefinition(overlay.type) : null;

  return (
    <div className="w-72 border-l border-zinc-200 dark:border-white/5 bg-zinc-50 dark:bg-zinc-900/50 flex flex-col overflow-hidden">
      {/* Header */}
      <div className="h-12 border-b border-zinc-200 dark:border-white/5 flex items-center justify-between px-4 shrink-0">
        <span className="text-sm text-zinc-600 dark:text-zinc-400">Properties</span>
        {overlay && (
          <button
            onClick={onRemove}
